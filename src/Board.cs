...
namespace ConwaysGameOfLife.Domain
{
    public class Board
    {
        ...
        private List<int> GetNeighbours(int index)
        {
            ...
            {
                return new List<int> { NeighbourService.ToRight(Bounds,x, y), ToRightBottom(x, y), ToBottom(x, y) };
            }
            ...
            {
                return new List<int> { NeighbourService.ToTop(Bounds, x, y), NeighbourService.ToRightTop(Bounds,x, y), NeighbourService.ToRight(Bounds,x, y) };
            }
            if (index == Bounds.GetBottomRight())
            {
                return new List<int> { NeighbourService.ToLeftTop(Bounds,x, y), NeighbourService.ToTop(Bounds, x, y), NeighbourService.ToLeft(Bounds,x, y) };
            }
            return new List<int> { NeighbourService.ToLeft(Bounds,x, y), NeighbourService.ToLeftTop(Bounds,x, y), 
                NeighbourService.ToTop(Bounds, x, y), NeighbourService.ToRightTop(Bounds,x, y), NeighbourService.ToRight(Bounds,x, y), 
